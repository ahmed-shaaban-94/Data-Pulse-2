...
- **2026-05-23** (P4 focused idempotency re-run — `2bc8ba7`, after PR #288) —
  Ran a focused re-run on branch `fix/email-queue-jobid-bullmq-compat` at `2bc8ba7`
  (PR #288: `fix(auth): use '-' separator in EmailQueueProducer jobId for BullMQ 5.x`).
  The §10.6 `EmailQueueProducer.deriveJobId` colon-separator defect is confirmed fixed:
  invite success path now reaches `tap.next`, idempotency store persists the replay
  record, and both previously-blocked signals are now LIVE-PROVEN:
  `idempotency_replay_total{route="POST:/api/v1/memberships/invite"} 1` and
  `idempotency_conflict_total{route="POST:/api/v1/memberships/invite"} 1`.
  Three sequential exercises (fresh invite → 201, replay → 201 same ID, conflict → 409)
  all produced correct client-visible responses. `http_error_5xx_total`
  no-regression check **FAILED**: counter moved from 0 to 2 post-exercise. Root
  cause is a **residual post-response race in `GlobalExceptionFilter`** — an async
  throw after `tap.next` emits the 201 causes the exception filter to attempt a
  second `res.json(500)` on an already-committed response, incrementing the counter
  without the client seeing a 500. This is a **separate defect from the BullMQ
  colon-separator issue PR #288 fixed** — PR #288 is not responsible for the 5xx
  counter increment. Evidence in `docs/observability/operator-validation-report.md §11`.
  **P4 verdict: PARTIAL (updated blockers)** — replay/conflict now LIVE-PROVEN;
  post-response race and `db_slow_query_total` / `db_rls_context_failure_total` remain
  open.
...
| P4 — Observability instrumentation | B | **PARTIAL** | Redaction and structured logging wired (T473/T474). API custom metrics emitting for exercised paths (PR #248). Worker job + queue metrics emitting (PR #251 / T596). Outbox metrics emitting (PR #253 PR-B-1 + PR #259 PR-B-2 / T595). `cross_tenant_rejection_total` and `db_rls_context_failure_total` emission confirmed wired at `tenant-context.guard.ts` (T475/T476 DONE). T483 exercised-path operator scrape **PASSED** 2026-05-21 for API/worker/outbox signals. PR #269 (PR-E) closed the `route="unknown"` exception-filter gap. **2026-05-22 re-run** on `857c178`: worker `:9091` boot UNBLOCKED by PR #278; `redis_command_duration_seconds`, `queue_lag_seconds`, `db_pool_in_use`, `db_pool_waiters` all now LIVE-SCRAPED with bounded labels. **2026-05-22 seeded-evidence run** on `de3bd9d` (HEAD = PR #286): six previously-absent SAFE signals confirmed LIVE — `auth_failure_total` (4 distinct causes), `suspicious_login_total{rapid_retry}`, `cross_tenant_rejection_total`, `tenant_context_failure_total{cross_tenant}`, `idempotency_in_progress_total`, `worker_processing_failure_total`. Bonus signal LIVE incidentally: `http_error_5xx_total` (via EmailQueueProducer defect). PR #284 (`b5b8d1b`) closed the §9.6 `db_migration_status` defect — gauge now correctly reports `applied=1, pending=0, failed=0`. **2026-05-23 focused re-run** on `2bc8ba7` (PR #288): `idempotency_replay_total` and `idempotency_conflict_total` now LIVE-PROVEN — PR #288 fixed the `deriveJobId` colon-separator defect (§10.6). Two signals still NOT live-proven: `db_slow_query_total` (no >500ms query exercised), `db_rls_context_failure_total` (unreachable from HTTP without source change). New open defect: post-response `ERR_HTTP_HEADERS_SENT` race in `GlobalExceptionFilter` inflating `http_error_5xx_total` on invite success paths (separate from PR #288 fix). P4 remains PARTIAL — see §4.C and `docs/observability/operator-validation-report.md §11`. |
...
3. **P4 full signal-catalogue live-scrape coverage (PARTIAL — explicit backlog)**
   ...
   **P4 signal backlog — not yet live-scraped:**
   ...
   | `idempotency_replay_total` | **Live-scraped and CORRECT** (PR #288 fix + 2026-05-23 focused re-run) | `{route="POST:/api/v1/memberships/invite"} 1` observed. The §10.6 EmailQueueProducer defect was the sole blocker; PR #288 (`2bc8ba7`) resolved it. Wiring at `idempotency.interceptor.ts:239` confirmed correct — replay record persisted on `tap.next` and served from cache on second call. See report §11. |
   | `idempotency_conflict_total` | **Live-scraped and CORRECT** (PR #288 fix + 2026-05-23 focused re-run) | `{route="POST:/api/v1/memberships/invite"} 1` observed via same-key different-body third request returning 409. See report §11. |
   | ~~`EmailQueueProducer.deriveJobId` defect~~ (§10.6 — closed by PR #288) | ~~Live production defect~~ — **FIXED** | PR #288 (`2bc8ba7`) changes separator in `deriveJobId` from `:` to `-`. All three callers (`enqueueInvitation`, `enqueuePasswordReset`, `enqueueEmailVerification`) now produce BullMQ-valid custom jobIds. Confirmed by live-proven `idempotency_replay_total` / `idempotency_conflict_total` in 2026-05-23 re-run. |
   | **Post-response `ERR_HTTP_HEADERS_SENT` race** (new — 2026-05-23 re-run) | **Residual open defect** | `GlobalExceptionFilter.catch` calls `res.json(500 body)` after `tap.next` has already emitted a 201 response to the wire. The async throw (source: email enqueue or post-emit hook firing after response commitment) causes Node.js `ERR_HTTP_HEADERS_SENT`; the filter still increments `http_error_5xx_total` before the Node.js error surfaces. Client receives correct 201 / 409; server-side counter is inflated. Observed: baseline 0 → post-exercise 2 on `{route="/api/v1/memberships/invite",status="500"}`. Recommended fix: guard `GlobalExceptionFilter.catch` with `if (res.headersSent) return;` before calling `res.json(...)`. Separate from PR #288 scope. See report §11.7. |
