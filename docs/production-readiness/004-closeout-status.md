...
- **2026-05-23** (P4 final focused evidence — `89c23aa`, after PR #289) —
  ...
  The post-response race documented in §11.7 is confirmed resolved: the `headersSent`
  guard (`if (response.headersSent) return;` added to `GlobalExceptionFilter.catch`)
  prevents `http_error_5xx_total` from being incremented when an async throw occurs
  after `tap.next` commits the response. Evidence: API started fresh from the `89c23aa`
  dist, `grep -c "headersSent" apps/api/dist/common/exception.filter.js → 1`
  (guard confirmed compiled). Three sequential exercises (fresh invite → 201, replay
  → 201 same ID `019e548b-81d1-7cc4-8132-52cebcd7f29f`, conflict → 409) all correct.
  Post-exercise scrape: `idempotency_replay_total{...} 1`, `idempotency_conflict_total{...} 2`,
  **`http_error_5xx_total` absent (0)**. API stderr `bin/api7.err.log` is empty — zero
  `ERR_HTTP_HEADERS_SENT` entries. 5xx no-regression check **PASSED**.
  `db_slow_query_total` and `db_rls_context_failure_total` remain explicitly deferred
  per §4.C — instrumentation wired but no HTTP path to exercise them without source
  change or slow-query synthetic. Evidence in `docs/observability/operator-validation-report.md §12`.
  **P4 verdict: PARTIAL-with-explicit-deferrals** — all instrumentation defects
  resolved; two signals explicitly deferred with documented rationale.
...
3. **P4 full signal-catalogue live-scrape coverage (PARTIAL — explicit backlog)**
   ...
   **P4 signal backlog — not yet live-scraped:**
   ...
   | `http_error_5xx_total` | **Live-scraped** (2026-05-22 seeded-evidence run — incidental); **no-regression PASS** (2026-05-23 final evidence) | `{route="/api/v1/memberships/invite", status="500"} 8` observed in §10 run (exercised by EmailQueueProducer defect). Counter increments and `route` template binding confirmed. In §11 run (after PR #288) false increments 0→2 observed due to post-response race. **2026-05-23 final evidence** (PR #289): `http_error_5xx_total` absent from 198-line post-exercise scrape — no false increments. `headersSent` guard confirmed effective. |
