...
- **Negative-balance signal (FR-024, R2)**: when the derived SUM for a key is `< 0`, the on-hand projection carries a **`negative_balance` flag**, and a **Prometheus counter** of negative-balance occurrences is incremented (labelled tenant/store, PII-free). This is the one new observability signal (plan §3.3).
