...
**RESOLVED `[GATED]` `packages/db` follow-ups (issue #465, closed 2026-06-03)** — both shipped after 009 closed; tracked + delivered via two PRs:
- **established-unit concurrency guard** — ✅ PR #470: migration `0016_inventory_unit_guard` adds `stock_movements_one_unit_per_product` (`EXCLUDE USING gist (store_id =, tenant_product_ref =, stocking_unit <>) WHERE tenant_product_ref IS NOT NULL`) — the path-independent backstop for the FR-022 race; self-guarding migration; app translates the `23P01` to a 400 `CrossUnitError`.
- movement **outbox emit** — ✅ PR #471: `OUTBOX_EVENT_TYPES.INVENTORY_MOVEMENT_CREATED` (`inventory.movement.created`) emitted in-transaction from all inventory write paths + the worker mirror; redelivered backfill emits exactly once (FR-033).
