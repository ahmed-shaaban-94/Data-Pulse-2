...
**Blocked on**: the **Money + Temporal Decision Gate** (`gate-money-temporal.md`) — a thin owner-decision prerequisite that resolves the constitution's four open Follow-up TODOs for transaction-level money, the per-entity timestamp required/optional set, and the payload-hash algorithm. The gate blocks **implementation/planning**, not this specification (see §11 Open Questions).
...
- No **returns/refunds workflow depth** — approvals, partial-refund UX, restocking, store-credit issuance, refund-to-tender routing. 008 models the *record* of a void/refund terminal event referencing a sale; the operational workflow is a later feature (see below). Deferred: **010** (pricing/promotions engine), **012** (returns/refunds workflow + sales reporting), per the project roadmap convention.
...
The following are unresolved **decisions for the owner**. They block **planning and implementation**, not this specification. All are consolidated in the **Money + Temporal Decision Gate** (`gate-money-temporal.md`), which MUST be resolved before `/speckit-plan`.

- **OQ-1 — Transaction-money representation** *(constitution Follow-up TODO #1 + Money-Tax-Rounding)*: For transaction-level money — line-tax representation (per-line vs summary), per-line vs invoice-level rounding, banker's-vs-half-up mode, the exact `numeric(p,s)` precision/scale, and the chosen money library/representation. (Catalog money is already pinned; only transaction money is open.)
- **OQ-2 — Tender / change / multi-tax modeling** *(Money-Tax-Rounding + §XIV)*: Whether 008 persists tender lines, change, and multi-tax breakdown at all, or defers them to a dedicated payments feature. (Payments today are contract-only / POS-Pulse-side; any backend tender persistence is payment-class under §XIV.)
- **OQ-3 — Per-entity timestamp required/optional set** *(constitution Follow-up TODO #2 + §X)*: Which of `occurredAt` / `receivedAt` / `processedAt` / `businessDate` / `sourceClockAt` are required vs optional on `sales` vs `sale_lines` vs void vs refund. (This spec fixes the field *set*; the gate fixes nullability.)
- **OQ-4 — Payload-hash algorithm** *(constitution Follow-up TODO #3 + §IX/§XIII)*: The exact hash algorithm and canonicalization for POS provenance (e.g., the constitution speculates "sha256 of canonical JSON?" — not yet decided).
- **OQ-5 — Concurrency posture confirmation** *(§III / Concurrency)*: Confirm the immutable-fact + idempotent-dedup posture (FR-070) and that no optimistic-concurrency `version` column is warranted for an append-only sale fact (LWW/posture must be *justified*, per §III — this spec justifies it; the owner confirms).
- **OQ-6 — Per-tenant bulk-sync bound** *(Per-Tenant Resource Isolation)*: The initial numeric defaults for the bulk offline-recovery sync bound (batch size ceiling, per-tenant ingestion rate / fair-sharing) layered on the inherited 001/004 posture. (FR-080 mandates the posture exists; the values are the owner's.)
- **OQ-7 — Sale-fact data class + retention window** *(constitution Follow-up TODO #7 + §XIV; spec SI-012)*: Confirm the sale-fact data class (default: **business-class**) and record its retention window (default: inherit 001 long-horizon insert-only audit-retention) and right-to-erasure note. Resolved at gate **D.3**; the §13 row XIV PASS is scoped "pending D.3."
...
| XIV. PII & Data Lifecycle Discipline | **Partially exercised; one item deferred-to-gate.** SI-009 — tender/payment data (if a future gate admits it) is payment-class under §XIV; v1 persists none; raw payloads never logged; redaction at the boundary. SI-012 — the new sale-fact entities are classified **business-class** with an inherited retention posture; the documented retention window + right-to-erasure note is constitution Follow-up TODO #7, routed to gate item **D.3** (decided before plan). PASS is therefore scoped to: classification stated (business-class), retention posture inherited-pending-D.3, no PII/payment persisted in v1 — *not* a claim that a final retention window is fixed in this spec. |

**Spec-level Constitution Check: PASS** for the obligations this spec fixes. The four constitutional **Follow-up TODOs** (money representation, timestamp required/optional, payload-hash algorithm, per-tenant quota) are **not violations** — they are owner decisions routed to the Money + Temporal Decision Gate (§11), which is the documented prerequisite before plan-level Constitution Check.
