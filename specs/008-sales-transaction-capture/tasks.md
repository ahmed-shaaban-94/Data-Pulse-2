...
- [ ] T033 [P] [US1] [TC] RED — `capture/dedup.spec.ts`: same `(tenant, sourceSystem, externalId)` ×N → exactly one `sales`, identical response, no double-apply; cross-tenant `externalId` collision isolated (FR-050, SC-003; SI-001). Predecessors: T013, T015. Acceptance: runs, fails.
...
- [ ] T036 [US1] [TC] GREEN-verify — extend the sweep (T015) for capture: cross-tenant read of the new sale → 404; RLS-bypass probe on `sales`/`sale_lines` ⇒ zero rows (SC-004). Predecessors: T035. Acceptance: sweep cases GREEN.
...
- [ ] T060 [P] [US5] [TC] RED — `idempotency/replay.spec.ts`: same sale ×5 + same `Idempotency-Key` → exactly one record, identical response, capture telemetry not incremented, duplicate-event-rate may increment (FR-050/052, SC-003). Predecessors: T035. Acceptance: runs, fails.
...
- [ ] T074 [US6] [TC] GREEN-verify — full cross-tenant/cross-store sweep (T015) GREEN across capture/void/refund/read incl. RLS-bypass probes on all four tables (SC-004). Predecessors: T073. Acceptance: sweep fully GREEN.
...
- **Total**: 36 tasks (T001–T002 setup; T010–T015 foundational incl. 2 `[GATED]`; T030–T074 the six user stories; T080–T082 worker; T090–T094 polish).
- **Per story**: US1=7, US2=4, US3=4, US4=4, US5=4, US6=5.
