...
**Gate-deferred decisions were not spec gaps — and are now RESOLVED.** Six+1 owner decisions (transaction money/tax/rounding, per-entity timestamp nullability, payload-hash algorithm, per-tenant bulk-sync bound, concurrency ratification, and sale-fact classification/retention) were routed to the **Money + Temporal Decision Gate** (`gate-money-temporal.md`) and tracked as §11 Open Questions OQ-1..OQ-7. These were constitution Follow-up TODOs (#1/#2/#3/#6/#7), intentionally owner-owned, not `[NEEDS CLARIFICATION]` ambiguities in the behavioral contract. The spec fixes the obligations (money is exact-decimal+currency, the timestamp field set, that a payload hash is retained); the gate fixed the open parameters. **The gate is now RESOLVED (2026-05-30): all of OQ-1..OQ-7 are closed (`gate-money-temporal.md` §Decisions Recorded), so the spec's WHAT is complete and `/speckit-plan` is no longer contingent on owner closure — it has run.** `/speckit-clarify` likewise surfaced no residual ambiguity.
