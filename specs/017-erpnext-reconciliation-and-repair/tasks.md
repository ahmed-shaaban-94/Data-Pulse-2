...
- [ ] T041 [US2] GREEN — `ErpnextReconciliationService.repairPosting` (the R1 state transition; re-uses the 015 resolve logic — extract/share a single copy, do NOT fork it; `SELECT … FOR UPDATE` on the 015 row to serialize concurrent repairs). Writes BOTH a platform `audit_events` row (FR-014 — a NEW in-transaction `INSERT INTO audit_events` on the same tx client: actor + tenant + store + target ref + outcome, no PII/raw payload; NOT the async `@Auditable` interceptor and NOT `insertAuditEvent`, neither of which is in-tx) AND the append-only `repair_attempt` record, atomic with the 015-row transition (all-or-nothing). Predecessors: T040. Acceptance: T040 GREEN; an `audit_events` row + a `repair_attempt` row are emitted in the same transaction.
...
- [ ] T053 [P] [TC] [US3] RED→GREEN — `repairStockMismatch` (`POST /api/v1/catalog/erpnext-reconciliation/runs/{runId}/results/{resultId}/repair`): an actionable class (`unmapped_store`→re-map, `quantity_divergence`→re-sync) writes — in ONE transaction (U1) — the append-only `repair_attempt`, the `erpnext_reconciliation_result.result_state` `open→repaired` transition, AND a platform `audit_events` row (FR-014, a NEW in-tx `INSERT INTO audit_events` — not `@Auditable`/`insertAuditEvent`); idempotent; never mutates the 009 ledger (a re-map drives the 014 admin flow, FR-013/016). `apps/api/test/catalog/erpnext-reconciliation/repair/stock-repair.spec.ts`. Predecessors: T052. Acceptance: GREEN; result_state + repair_attempt + audit_events all written atomically.
...
## Future / out-of-scope (NOT 017 v1 — traceability only)

> These map to the execution-map's non-dispatchable `017-STOCK-VIEW-CONTRACT` /
> `017-SCHEDULED-RUNS` slices. Listed here so the map's `task_ids` resolve; they
> are **not** v1 tasks and have no acceptance criteria yet.

- [ ] T100 [GATED] (FUTURE — non-dispatchable) The live connector→DP2 ERPNext-Bin **view contract** that activates US3's live stock read (a future `[GATED]` `packages/contracts` slice authored WITH the connector repo, ADR 0008). 017 v1 ships against the stub-tolerant seam (research R3); this is its activation. Blocked: the connector repo + view contract do not exist yet.
- [ ] T101 (FUTURE — later wiring) **Scheduled** reconciliation runs — a BullMQ repeatable job enqueuing the SAME v1 run processor on a per-tenant cadence (research R5: v1 is on-demand only; scheduling is wiring over the existing idempotent processor, not new run logic).
