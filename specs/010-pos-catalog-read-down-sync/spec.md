...
**Payload**
- **FR-050**: Each sellable row MUST carry: `product_id`, `sku`, `name_ar` (NOT NULL), `name_en` (nullable), aliases (list), `price` as `{ amount, currency_code }`, `tax_category`, `unit_pack_label` (nullable), a resolved **active** flag, `controlled_substance`, `prescription_required`, and a per-row cursor/`row_version`.
...
- **Resolved Sellable Catalogue Row**: the projection a POS terminal reads — one per sellable product for the store, derived from `Tenant Catalog ⊕ Store Override` (003 §6.4), filtered by the sellable rule (FR-040). Carries the FR-050 payload. Not a new authoritative table; a read projection.
