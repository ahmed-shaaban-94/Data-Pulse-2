...
| F5 | LOW | tasks T004/T005 migration number `00NN` | Migration number is a placeholder (expected `0022`) pending `main` state at gate time. **Cross-spec collision: 021-product-master-reconciliation ALSO reserves indicative `0022`.** | Correct for a no-implement pass; the gate-time task confirms the real number. The collision is documented in both `020/plan.md` and `021/plan.md` (whichever SCHEMA slice authors second takes the next free number). Carried in research "Open items." No change. |
