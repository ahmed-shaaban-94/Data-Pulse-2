...
| **III — Backend authority (authorization)** | Privileged admin surface. | Issue/rotate/revoke/disable are privileged ops — gated by `DashboardAuthGuard` + `TenantContextGuard` + `RolesGuard` `@Roles("owner","tenant_admin")` (default-deny → 404), the **014/017 controller precedent** (FR-005b). `DashboardAuthGuard` alone (authenticated-but-any-role, incl. `dashboard_api` bearer) is INSUFFICIENT for credential issuance. ✅ |
...
│   ├── connector-registration.controller.ts   # admin surface: DashboardAuthGuard + TenantContextGuard + RolesGuard @Roles("owner","tenant_admin") [iff REST]
