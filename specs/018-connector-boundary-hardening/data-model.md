...
| **CHECK (preflight-gated)** connector-token consistency | `scope='connector'` iff `connector_registration_id IS NOT NULL`. Added only after a preflight of existing connector tokens; legacy rows → backfill-or-defer per R3. |
