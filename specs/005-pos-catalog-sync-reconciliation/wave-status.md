...
| **T560 perf-budget threshold brittleness** — SC-008 `p95 <= 500ms` assertion in `capture-latency.spec.ts` | POLISH PR #351 | **Observed flake on 2026-05-28**: same-SHA back-to-back CI runs on `main` produced p95 = **760ms (FAIL)** then p95 = **446ms (PASS)** — a 1.7× spread on identical code, from GitHub-hosted runner variance against the 50k-product / 100k-alias full-scale fixture. Headroom on the passing run was **10.8%** (446 vs 500). The spec's `dockerSkipped` gate (line 440-448) only handles the Windows-host-no-Docker case; there is **no runner-variance escape hatch** in CI. Expected recurrence: ~5–15% of runs. **RESOLVED via slice `005-WAVE1-T560-PERF-FLAKE` (executed 2026-05-28).** Diagnostic-first per the slice contract: the RED-run distribution (CI run 26608290112) showed **whole-run slowness** (p50=185 / p95=1372 / p99=2351 ms) vs PASS runs (p50~100 / p95~160 ms) — the median itself ~1.8× elevated, i.e. runner contention, not cold-start outliers. That tripped the slice's whole-run-slow stop → warmup-discard ruled out → chose **mechanism (b): report-only gate.** The two build-failing `expect(p95≤500)/(p99≤1000)` assertions are now a logged WITHIN/OVER soft-check; the **SC-008 threshold values (500/1000) are UNCHANGED** (only build-blocking removed — §IV-respecting) and per-call 2xx success stays build-gating. **Follow-up (open):** authoritative SC-008 enforcement belongs in a dedicated, contention-free perf environment (not shared GitHub-hosted CI runners) — recorded here as the proper future home for the SLA gate, not yet built. Distinct test category from `005-WAVE1-METRICS-MISMATCH-FOLLOWUP`. |
...
**Resolved follow-ups** (formerly "Outstanding"; both closed as of 2026-05-29):
