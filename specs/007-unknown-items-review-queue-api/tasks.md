...
**Both `[SIGN-OFF]` decisions MUST be recorded (in this tasks.md or `wave-status.md`) before the dependent GREEN tasks execute.**

- **T002 default: TIGHTEN.** Because FR-007 is a MUST NOT and the shipped list is the review queue, the default is that the list (shipped + new) stops returning `sale_context`. "Leave the shipped list unchanged" is NOT a compliant default — it would ship an FR-007 violation; choosing it requires an explicit FR-007 *waiver* recorded by the human.
- **T003 default: ISOLATE.** For idempotency, "isolate" (only new reopen/bulk-dismiss carry the key; shipped link/create/dismiss keep their monotonic-guard no-duplicate-effect) IS compliant — FR-063's reworded two-strength model is satisfied without touching shipped ops. Retrofitting the key onto shipped ops is the optional enhancement.
...
- [ ] T002 [SIGN-OFF] Record the `sale_context` tightening decision (research §R1): the shipped `tenantAdminListUnknownItems` and all new 007 read ops MUST stop returning `sale_context` (FR-007 MUST NOT; the shipped list IS the review queue). The decision is **how**: **(a) tighten now** — switch the list response to `ReviewQueueItem` in this slice; or **(b) tighten with a deprecation note** — version-bump the shipped response shape with a documented window. Choosing to **leave `sale_context` on the list requires an explicit FR-007 waiver** and is NOT the default. Write the verdict + rationale into `wave-status.md`. Predecessors: none. Acceptance: a recorded decision exists; **default = tighten** (`(a)`) if none — never "leave unchanged." This gates T032 / T042.
- [ ] T003 [SIGN-OFF] Record the idempotency-key retrofit decision (research §R6): **(a) retrofit** `Idempotency-Key` onto shipped link/create/dismiss, or **(b) isolate** — only new reopen/bulk-dismiss carry the key. Write the verdict into `wave-status.md`. Predecessors: none. Acceptance: recorded decision; default `(b) isolate`. This gates whether T060–T062 add key-replay assertions.
...
- [ ] T023 GREEN — add a `toReviewQueueItem(row, canSeeProduct)` projection helper in `apps/api/src/catalog/unknown-items/unknown-items.controller.ts` (or a sibling `dto/review-queue-item.dto.ts`) that returns the shipped `UnknownItem` shape minus `sale_context`, with FR-001a conditional suppression. (Foundational — used by US1/US2/US3, so no single story label.) Predecessors: T022. Acceptance: T022 GREEN.
...
- [ ] T053 [US7] GREEN — implement `reopenUnknownItem(id)` in `apps/api/src/catalog/reconciliation/reconciliation.service.ts`: tenant-wide authority check (→ `forbidden` in-scope / `not-found` out-of-scope), `resolved`→already-reconciled guard, `pending`-sibling→already-pending guard, else INSERT a fresh `pending` row via the 005 FR-005 capture path; emit audit for both the reopen action and the fresh capture. Predecessors: T050, T051, T052, T010, T021. Acceptance: T050–T052 GREEN.
- [ ] T054 [US7] GREEN — add the POST `/{id}/reopen` route to `reconciliation.controller.ts` mapped to `tenantAdminReopenUnknownItem` (T010), `cookieAuth`, `Idempotency-Key` header wired to the existing interceptor, Zod `.strict()` (empty/`{}` body — no body-supplied tenant/store), `@Auditable` per the existing convention. Predecessors: T053. Acceptance: T050–T052 still GREEN; OpenAPI conformance passes.
