...
> **006 introduces NO new entities, columns, indexes, or constraints.** This document is a **pointer doc**: it lists the entities 006 consumes from 005 / 003 / 001, identifies the user-visible projections the future Wave A API feature must produce from them (per spec FR-001a, FR-020..022 + FR-021a), and explicitly disclaims any schema authoring for 006. Wave A's plan will produce its own data-model.md against this contract.
...
| `unknown_items.sale_context` (JSONB column) | 003 §8 | `packages/db/src/schema/catalog/unknown-items.ts:45` (per 005 plan §4.1) | **Read but NOT surfaced in v1** per FR-021a. The future Wave A query layer reads the row but the response projection MUST NOT include any field from this column. |

**Net**: zero schema authorship by 006.

---

## 2. User-visible projection obligations (for Wave A)

The future Wave A API feature is **obligated** to produce the following projections from the entities above. These are obligations, not designs — Wave A's plan will pin the wire shape (per Constitution §IV "API responses MUST NOT return raw database entities"). 006 only fixes what fields must / must not appear.
...
| `captured_at` | timestamp | `unknown_items.created_at` (or 005's chosen name) | Reviewer's local timezone display is a Wave B (Impeccable) concern. |
...
| `dismiss` | `unknown_item_id` (optionally a free-form `reason` if Wave A chooses to support one — out of scope here) | `tenant_id`, `store_id`, `lifecycle_state`, audit fields |
...
On success, the response body shape is Wave A's decision. The minimum 006 requires:
...
- Idempotent on retry (per Constitution §XI; Wave A applies the existing idempotency interceptor).

On failure, the canonical error envelope (`{ error: { code, message, request_id, details? } }` per Constitution §III) MUST use one of the eight category values enumerated in [research.md §R4](./research.md):

`validation` | `target-unavailable` | `alias-conflict` | `idempotency-token-mismatch` | `already-reconciled` | `already-terminal` | `not-found` | `system-failure`
...
- 001's `runWithTenantContext` / `withTenant` helpers — Wave A will use these for every query.

Wave A's data-model.md will document its query patterns against these policies; 006 only commits that the queue surface MUST use them with no bypass.
...
- **Reopen-specific subject** — Wave A may choose `unknown_item.reopened` as a distinct audit subject or may rely on the captured + correlated `unknown_item.captured` event for the fresh-pending row. 006 takes no position; both satisfy FR-110.
...
006 contributes **zero schema artifacts** and instead pins **wire-projection obligations** the future Wave A feature must honor. The obligations are testable from the spec's acceptance scenarios (§5) and isolation requirements (§7). Wave A's data-model.md will translate these obligations into concrete DTOs against 005's then-current service surface.
