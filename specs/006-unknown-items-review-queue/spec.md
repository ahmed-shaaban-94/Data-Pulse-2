...
- Q: What is the maximum count of items per bulk-dismiss submission (FR-070)? → A: 200 items max per submission. The platform MUST enforce this ceiling; submissions above it are rejected with a `validation` category outcome (per FR-100). Bulk dismiss is a UX-layer batching of 005's per-item dismiss — no new lifecycle transition, no new audit subject (see FR-070a). (FR-070, FR-070a updated.)
- Q: Which actors may perform the "reopen" action (US8 / FR-061)? → A: Tenant-wide actors only — Tenant Admin and Tenant Owner. Store-scoped operators MAY dismiss within their scope but MUST NOT reopen. Reopen is a correction-of-prior-judgment action that warrants tenant-level authority for second-pair-of-eyes review. Authority-failure category is **scope-dependent**: out-of-scope → `not-found` per 005 SI-004; in-scope-but-no-reopen-authority → `forbidden` per FR-100, consistent with Constitution §II / §XII allowance of `403` for "insufficient-role within an already-resolved active tenant." (US8, FR-061, FR-062a, FR-100, §4 Actors updated.)
- Q: What detail is surfaced when an authorized reviewer filters the queue to `resolved` or `dismissed` items? → A: `dismissed` items show full in-scope detail (identifier metadata, capture store, source system, capture and dismissal timestamps, dismissing actor). `resolved` items show identifier metadata, capture store, source system, timestamps, and resolution action (`linked` / `created`); the linked/created product reference is shown to the reviewer only if the actor has authority to see that product per 005 SI-004 — otherwise the product identity is not visible to the reviewer (the suppression mechanism is a future API + UI choice). (FR-001a added.)
- Q: Does v1 surface (a) optional POS descriptive metadata from 005's `unknown_items.sale_context jsonb` and (b) advisory candidate-match hints inside the inspection view? → A: (a) Descriptive metadata MUST NOT be surfaced in v1 — reviewers rely on identifier metadata alone, consistent with 005 FR-006's non-identity / advisory-only framing. (b) The candidate-match hint **decision** is delegated to the future API + UI features (because the query cost warrants future-API-feature cost analysis and the false-positive UX risk warrants future-UI-feature critique) — **but the safety boundaries on candidate sourcing remain MUST**: in-scope only, no auto-link, no pre-select, no leakage. (FR-021 and FR-080 updated — see external-review revision below.)
- **Revision note 2026-05-24 (after external review)**: FR-080's display decision was reverted from `MUST` to `MAY` — the safety boundaries stayed `MUST`. Rationale: the prior `MUST` baked a non-trivial query (future API feature) and UX cost (future UI feature) into the spec without acceptance-scenario or cost analysis backing. The `MAY` form preserves the safety floor while letting the future API + UI features decide v1 vs v2.
- **Revision note 2026-05-24 (after external review)**: The `already-terminal` category introduced in the original clarify session was collapsed into `already-reconciled` with an optional `details` discriminator. Rationale: from the user's seat the two cases admit the same remedy (refresh, see new state, decide if a sibling action applies); a distinct closed-vocabulary value did not earn its keep against the cost of propagating an extra enum through every contract consumer.
- **Revision note 2026-05-24 (after external review)**: A new category `forbidden` was added to FR-100 to carry the in-scope-but-no-reopen-authority case (FR-062a). Constitution §II / §XII explicitly permit `403` for "insufficient-role within an already-resolved active tenant"; the prior wording categorised this as `validation` which was semantically wrong (input was well-formed; authority was not).
...
**Platform Operator is intentionally not listed as a queue actor.** FR-005 and SI-001 both bar platform operators from any tenant-scoped surface of the review queue. Aggregate platform-level operational signals (queue depth, capture rate) are governed by 003 §9 / 005 §6.9 and are out of scope here. 006 introduces no new platform read surface.
...
3. **Given** U is already `resolved` or `dismissed`, **When** the reviewer attempts to dismiss U again, **Then** the platform refuses with the `already-reconciled` category (carrying a `details` discriminator indicating prior state per FR-100) distinguishable from a successful dismissal; U is unchanged.
...
3. **Given** U is `resolved`, **When** any reviewer attempts to "reopen" U, **Then** the platform refuses with the `already-reconciled` category (with `details` indicating `resolved` prior state per FR-100); the reviewer is guided to undo via alias retirement / product correction in a separate future workflow rather than via the review queue.
4. **Given** a store operator scoped to S1 (where U was dismissed) attempts to reopen U, **When** the action is submitted, **Then** the platform refuses with `forbidden` (per FR-062a + FR-100) — store-scoped operators cannot reopen even within their own scope. The response carries enough context for the reviewer to understand "tenant-wide authority required" but nothing more.
5. **Given** a store operator scoped only to S1 attempts to reopen an item dismissed at S2, **When** the action is submitted, **Then** the platform returns `not-found` (per 005 SI-004 + FR-062a) — the operator cannot tell whether the item exists. (Distinct from scenario 4 because the operator has no read authority for the row at S2 in the first place.)
...
Every user-visible failure from the queue is presented as one of a small set of categories the user can reason about, without leaking forbidden detail. Failure categories per FR-100 (revised 2026-05-24): validation, target-unavailable, alias-conflict, idempotency-token-mismatch (rarely user-facing here), already-reconciled (covering both race-loser and the static-state-mismatch case of dismiss / reopen on a terminal row — distinguished via optional `details.prior_state`), not-found (cross-tenant or out-of-scope), forbidden (in-scope authority failure, e.g., FR-062a), system-failure. Messages are calm, deterministic, and never reveal whether an out-of-scope record exists.
...
**Independent Test**: Exercise each failure category from FR-100 (revised 2026-05-24): validation, target-unavailable, alias-conflict, idempotency-token-mismatch, already-reconciled (both race and static-state-mismatch sub-cases via `details.prior_state`), not-found, forbidden, system-failure. For each, verify: (a) the user sees a category that is meaningful but not over-specific; (b) no out-of-scope record existence is revealed; (c) the failure is recorded as an audit event where 005 requires it (per 005 FR-082).

**Acceptance Scenarios**:

1. **Given** any user action that 005 rejects, **When** the rejection surfaces in the queue, **Then** the user sees a category from FR-100's closed set (005 FR-091's seven categories plus `forbidden`); no category leaks the existence of out-of-scope records.
...
- **Review action fails after the user has already seen stale queue data**: The action returns the appropriate category outcome (`already-reconciled` — with `details.prior_state` if the row was already terminal — or `not-found`); the queue refreshes; the user is not penalised for acting on stale data (per FR-090 below).
...
- **FR-001a — Detail surfaced for terminal items (user-visible outcomes only)**: When an authorized reviewer filters or navigates to `dismissed` items, the reviewer MUST be able to see, for each in-scope row: identifier metadata, capture store, source system, capture timestamp, dismissal timestamp, and dismissing actor. No product or alias detail applies because no product was touched. When an authorized reviewer filters or navigates to `resolved` items, the reviewer MUST be able to see, for each in-scope row: identifier metadata, capture store, source system, capture timestamp, resolution timestamp, resolving actor, and `resolution_action` (`linked` / `created`). For the **linked or created product** referenced by a `resolved` row, the user-visible outcome MUST be: (a) **if the actor has authority to see that product**, the product's identifying detail (name, primary identifier) is visible to the reviewer; (b) **if the actor lacks that authority** (e.g., the product has since been retired into a state the actor cannot see, or product visibility is otherwise revoked while the unknown-item row remains in scope), the product's identifying detail MUST NOT be visible to the reviewer — but the row itself MUST remain visible (the existence of the unknown-item record in the actor's scope is already authoritatively established, per 005 SI-004). The **mechanism** by which the platform suppresses product detail (omitted field, masked-detail marker, etc.) is a future API + UI design choice — 006 only pins the user-visible outcome.
...
- **FR-036**: When the queue view is older than the underlying data (a "stale" view), an interaction MAY succeed against fresher state, MAY fail with `already-reconciled` (with `details.prior_state` indicating the actual terminal state when applicable) or `not-found`, or MAY refresh the view. The user MUST NOT be penalised for acting on a stale view (no destructive default).
...
- **FR-062a — Reopen authority restricted to tenant-wide actors**: Only Tenant Admin and Tenant Owner principals MAY initiate a reopen action. Store-scoped operators (Store Operator / Store Manager) MUST NOT be able to reopen unknown items, even within their own store scope. The category mapping for a refused reopen attempt is **scope-dependent**: (a) for an item **outside** the operator's store scope, the platform MUST respond with `not-found` per 005 SI-004 — the operator cannot tell whether the item exists; (b) for an item **inside** the operator's store scope (so the operator already has read authority and could see the row in the queue), the platform MUST respond with `forbidden` (FR-100), consistent with Constitution §II / §XII which permit `403` semantics for "insufficient-role within an already-resolved active tenant." The `forbidden` response MUST NOT leak any information beyond "this action requires tenant-wide authority." The rejection (either category) MUST be auditable per FR-111.
...
- **FR-070**: Bulk **dismiss** MAY be offered when (a) every selected item is `pending`, (b) every selected item is within the actor's scope, and (c) the count per submission is at most **200 items**. The platform MUST enforce this 200-item ceiling — submissions above it MUST be rejected with the `validation` category outcome (per FR-100) and MUST produce no partial-success state (all-or-nothing at the *ceiling* boundary; per-item outcomes inside a ≤ 200 submission are governed by FR-100 + SC-008 mixed-success semantics). Bulk dismiss is the only bulk action this spec admits for v1.
- **FR-070a — Bulk dismiss is a UX-layer batching of 005's per-item dismiss**: 006 does **not** introduce a new lifecycle transition, a new audit subject, or a new dismiss semantic. Each bulk-dismiss request MUST decompose, at the implementation layer, into N per-item dismiss operations under 005's existing §6.7 contract — same lifecycle write (`pending → dismissed`), same `resolution_action = dismissed` per 005 FR-003, same per-item audit event per 005 FR-080. The bulk-dismiss action is a **UX affordance**, not a 005-layer extension; if 005's per-item dismiss semantics shift in a future Wave, the bulk affordance shifts with them at no additional spec change in 006. The 200-item ceiling is a UX/operational constraint specific to this batching affordance, not a 005-layer limit.
...
- **FR-100**: The user-facing experience MUST surface failures using the categories defined in 005 FR-091, extended by 006 with **one** new category: `forbidden` (for authorization failures inside an already-resolved tenant context — see FR-062a and Constitution §II / §XII which both explicitly permit `403` semantics in this case). The full closed set: `validation`, `target-unavailable`, `alias-conflict`, `idempotency-token-mismatch` (rarely user-facing here), `already-reconciled`, `not-found`, `forbidden`, `system-failure`. The `already-reconciled` category absorbs both the concurrent-race case (somebody else just reconciled it) and the static-state-mismatch case (the row was already in a terminal state when the action was attempted); responses MAY carry a richer `details` object indicating prior state so clients can distinguish, but the closed-set vocabulary stays at 005 FR-091's seven values plus `forbidden`.
...
- **SC-006**: 0% of queue interactions leave the user with a stale-view ambiguity that produces an incorrect lifecycle outcome (per FR-090 / 005 SC-007). Concretely: every action either succeeds idempotently or returns one of the `already-reconciled` (race or static-state-mismatch via `details.prior_state`) / `not-found` / `forbidden` categories, never a silently-incorrect "success".
...
- **SC-008**: Bulk dismiss, where offered, completes with the same per-item correctness guarantees as single dismiss: every successfully dismissed item is `dismissed` and audited; every rejected item (`already-reconciled` for already-terminal siblings, `not-found` for out-of-scope, etc.) is reported per FR-100 without affecting other items in the batch. The 200-item ceiling is enforced at the batch boundary (per FR-070 — all-or-nothing rejection); per-item outcomes inside a valid batch are mixed-success. (Measured by the bulk-dismiss integration test under mixed-success workloads.)
...
- **v1 advisory scope**: optional POS descriptive metadata travelling inside 005's `unknown_items.sale_context jsonb` MUST NOT be surfaced (FR-021a). The candidate-match hint in the inspection view is permitted but not mandated for v1 (FR-080 MAY) — when surfaced, it MUST honor the in-scope safety boundaries (no out-of-scope sourcing, no auto-link, no pre-select, no leakage). Future surfacing of descriptive metadata is a separate opt-in feature.
...
None blocking. The four material ambiguities surfaced during `/speckit-clarify` (2026-05-23) — bulk-dismiss ceiling, reopen authority, detail surfaced for terminal items, and v1 scope of advisory surfaces — are all resolved in the Clarifications section near the top of this spec. The external-review pass on 2026-05-24 revised three of those resolutions (FR-080 MUST → MAY for the display decision; `already-terminal` collapsed into `already-reconciled` with a `details` discriminator; `forbidden` added to the FR-100 closed set to carry FR-062a's in-scope authority case); those revisions are also captured in the Clarifications section. The spec consumes 005 / 003 semantics as-is, and the earlier point of friction — what "reopen" means against a monotonic lifecycle — remains resolved deterministically by modeling reopen as a fresh-`pending`-record action per 005 FR-005 (see §9 Assumptions and US8). Any remaining UI-level questions (exact age buckets, pagination thresholds, listing-latency targets, message wording, visual treatment of advisory markers, accessibility behavior, and whether the candidate-match hint surfaces in v1 vs v2) are intentionally routed to Impeccable (§11) and to the future API + UI features rather than this spec.
